...
public class AWSSecretRepositoryTest extends PowerMockTestCase {
    ...
    @Test(description = "Positive test case for init() method for secret retrieval when encryption is enabled.")
    public void testInitEncryptedEnabledSecretRetrievalPositive() {
        ...
        Assert.assertTrue(Whitebox.getInternalState(awsSecretRepositorySecretRetrieval, "encryptionEnabled"));
    }

    @Test(description = "Positive test case for init() method for secret retrieval when encryption is not enabled.")
    public void testInitEncryptedNotEnabledSecretRetrievalPositive() {

        when(AWSVaultUtils.getProperty(properties, ENCRYPTION_ENABLED)).thenReturn("false");
        awsSecretRepositorySecretRetrieval.init(properties, anyString());

        Assert.assertFalse(Whitebox.getInternalState(awsSecretRepositorySecretRetrieval, "encryptionEnabled"));
    }

    @Test(description = "Positive test case for init() method for secret retrieval when " +
            "encryption enabled property is empty.")
    public void testInitEncryptedEnabledEmptySecretRetrievalPositive() {

        when(AWSVaultUtils.getProperty(properties, ENCRYPTION_ENABLED)).thenReturn(null);
        awsSecretRepositorySecretRetrieval.init(properties, anyString());

        Assert.assertFalse(Whitebox.getInternalState(awsSecretRepositorySecretRetrieval, "encryptionEnabled"));
        verify(logger).warn("Encryption enabled property has not been set. It will be taken as false by default.");
    }
    ...
    public void testGetSecretEmpty() {
        ...
        verify(logger).warn(Mockito.contains("is null or empty"));
